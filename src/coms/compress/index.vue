  ...
  <div class="re-compress-container">
    <div class="re-upload">
      <div class="re-upload-box" ref="upload_box">
        ...
        <div class="re-upload-text">{{ $ils8.transf('compress.tip') }}</div>
    ...
    <div class="re-compress-show" v-show="show">
      <p class="re-compress__text">
        {{
          `${$ils8.transf('compress.selectText')}：${file.name}，${$ils8.transf('compress.sizeText')}：${file.size}M`
        }}
      </p>
      <p class="re-compress__text">{{ `${$ils8.transf('compress.oldPicSizeText')}：${file.width}x${file.height}` }}</p>
      <p class="re-compress__text">
        {{ `${$ils8.transf('compress.newPicSizeText')}：${newFile.width}x${newFile.height}` }}
      </p>
      <p class="re-compress__text">
        {{ $ils8.transf('compress.success') }}，
        <a :href="newFile.imageUrl" :download="file.name">{{ $ils8.transf('compress.download') }}</a>
      </p>
    </div>
  </div>
</template>
